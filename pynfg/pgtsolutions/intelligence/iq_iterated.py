...
def iq_MC_iter(G, S, X, M, d, integrand):
    ...
    for s in xrange(S): #sampling S sequences of policy profiles
        for tout in xrange(T0, T+1): #sampling a sequence of policy profiles
            # gather list of decision nodes in time tout
            Xlist = [d for d in G.time_partition[tout] if \
                                                isinstance(d, DecisionNode)]
            for d in Xlist: 
                    d.randomCPT(setCPT=True) #drawing current policy
                    for dd in G.bn_part[d.basename][tout::]: 
                        dd.CPT = d.CPT #apply policy to future timesteps
            for p in G.players: #find the iq of each player's policy in turn
                iq[p][tout-T0] = iq_calc_iter(p, G, X, M, d, start=tout)
    funcout[s] = integrand(G) #evaluate integrand G(s) and assign to funcout
    intel[s] = iq #assign 
    return intel, funcout
                        
def iq_calc_iter(player, G, X, M, d, start):
    ...
    for x in range(X):
        ...
        npvreward = (G.npv_reward(player, start, d) + (x-1)*npvreward)/x
    altnpv = [0]*M
    G1 = copy.deepcopy(G)
    Ylist = [j for j in G1.partition[player] if j.time == start]
    for m in range(0,M): #Sample M alt policies for the player
        for d in Ylist: 
            d.randomCPT(setCPT=True) #random altpolicy for each DN in time tout
        ...
        altnpv[m] = G1.npv_reward(player, start, d) #get alt npvreward
    worse = [j for j in altnpv if j<=npvreward[player]] #alts worse than G
